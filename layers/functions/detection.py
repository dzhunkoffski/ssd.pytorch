...
#         """No gradient needed for detection layer."""
#         return (None,) * 8  # Match number of forward arguments
...
class Detect(Function):
    @staticmethod
    def forward(ctx, num_classes, bkg_label, top_k, conf_thresh, nms_thresh, 
                loc_data, conf_data, prior_data):
        """
        Args:
            ctx: Context object
            Configuration params (passed directly):
                num_classes, bkg_label, top_k, conf_thresh, nms_thresh
            Tensors:
                loc_data: [batch, num_priors*4]
                conf_data: [batch*num_priors, num_classes]
                prior_data: [1, num_priors, 4]
        """
        # Store config params in context
        ctx.num_classes = num_classes
        ctx.background_label = bkg_label
        ctx.top_k = top_k
        ctx.nms_thresh = nms_thresh
        ctx.conf_thresh = conf_thresh
        ctx.variance = torch.tensor(cfg['variance'], device=loc_data.device)

        num = loc_data.size(0)
        num_priors = prior_data.size(0)
        output = torch.zeros(num, num_classes, top_k, 5, device=loc_data.device)
